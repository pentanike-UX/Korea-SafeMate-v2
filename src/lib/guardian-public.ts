...
export function mergePublicGuardian(g: GuardianProfile): PublicGuardian | null {
...
export function listPublicGuardians(): PublicGuardian[] {
  return mockGuardians.map(mergePublicGuardian).filter(Boolean) as PublicGuardian[];
}
